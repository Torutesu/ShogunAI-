...
export function PlanStage({ state, onPersist }: { state: OnboardingState; onPersist: Persist }): JSX.Element {
  ...
  return (
    <section className="onb-stage">
      ...
      {state.plan === "pro" ? <div className="onb-fact"><strong>{t.obKeyTitle}</strong><span>{saved ? t.keySaved : t.obKeyBody}</span>{!saved ? <div className="onb-keyrow"><input type="password" autoComplete="off" value={key} placeholder={t.keyPlaceholders.anthropic} onChange={(event) => setKey(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") saveKey(); }} /><button className="onb-button" type="button" disabled={!key.trim()} onClick={saveKey}>{t.keySave}</button></div> : null}</div> : null}
