...
pub mod mac {
    ...
    /// The live exclusion categories for the onboarding "reads" step (issue #6). Read from the
    /// process-wide policy so the flow shows what is actually enforced — the UI must never hardcode
    /// this list, or it would misrepresent today's behaviour. Fails closed to an empty list before
