...
/// Match the hover tap recovery cadence without keeping AppKit's main thread waiting.
...
/// AppKit monitor tokens and their handlers are main-thread-only. NSEvent retains the handler,
/// and we retain both ownership handles so revoke/regrant can remove them deterministically.
#[derive(Default)]
...
fn current_modifier_flags() -> usize {
    ...
    // CoreGraphics' combined-session state is safe from the voice worker and uses the same
    // kCGEventFlagMask bit positions as NSEvent modifierFlags for these modifier keys.
...
/// Remove every retained global monitor token. AppKit requires this to run on its main thread.
fn remove_global_monitors_main(app: &tauri::AppHandle) {
    ...
    // SAFETY: each token came from NSEvent addGlobalMonitor and is removed exactly once.
    unsafe {
...
/// Must run on AppKit's main thread. Local fallback is always available while SHOGUN is focused.
fn install_local_monitors_main(app: &tauri::AppHandle) {
    ...
    // SAFETY: AppKit main thread. Local monitor blocks must pass events through unchanged.
    unsafe {
...
/// Install only missing global monitor handlers. Caller has already checked Accessibility trust.
fn install_global_monitors_main(app: &tauri::AppHandle) {
    ...
    let after = before.after_attempt(installed_down, installed_up);
    if !after.needs_recovery() {
        eprintln!("[voice] global hold-to-talk monitors installed");
    }
}

/// Reconcile trust changes and monitor ownership on AppKit's main thread.
fn reconcile_global_monitors_main(app: &tauri::AppHandle) {
    ...
    let trusted = crate::axcache::ax_trusted_silent();
    match global_monitor_action(trusted, global_monitor_state()) {
...
mod tests {
    ...
    fn denied_startup_keeps_empty_global_monitor_ownership() {
        let action = global_monitor_action(false, GlobalMonitorState::default());

        assert_eq!(action, GlobalMonitorAction::Keep);
    }
    ...
    fn trusted_partial_registration_installs_only_missing_handler() {
        ...
        let action = global_monitor_action(true, state);

        assert_eq!(action, GlobalMonitorAction::InstallMissing);
    }
    ...
    fn revoked_accessibility_removes_retained_global_tokens() {
        let state = GlobalMonitorState::default().after_attempt(true, true);
        let action = global_monitor_action(false, state);

        assert_eq!(action, GlobalMonitorAction::RemoveAll);
    }

    #[test]
    fn regranted_accessibility_reinstalls_cleared_global_tokens() {
        let action = global_monitor_action(true, GlobalMonitorState::default());
