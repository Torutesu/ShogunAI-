...
/// shipped binary carrying the operator's key can be extracted, spend caps become unenforceable,
/// and invariant 5 scopes this credential to the Batch lane. The relay has no live-Messages
/// endpoint yet, so release reports the honest needs-key state instead of spending the raw
/// company key from the user's machine.
#[cfg(debug_assertions)]
