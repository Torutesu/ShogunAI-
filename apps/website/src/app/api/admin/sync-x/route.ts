...
/**
 * POST /api/admin/sync-x  (auth: x-admin-token header)
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Log the detail server-side; return a generic code so bridge internals
    // (URLs, upstream status) don't leak in the response body.
    console.error('sync-x error:', e);
    return NextResponse.json({ ok: false as const, error: 'sync_failed' }, { status: 500 });
  }
}
