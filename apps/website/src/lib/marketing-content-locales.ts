...
const deFeatures: readonly MarketingDetail[] = [
  {
    ...
    title: 'Ein privates Gedächtnis für den Kontext hinter deiner Arbeit',
    description: 'ShogunAI erfasst Arbeitskontext auf deinem Mac und macht ihn als strukturierte, durchsuchbare Zeitleiste nutzbar – ohne Screenshot-Archiv.',
    intro: 'Wichtige Informationen liegen selten in einem Dokument. Sie verteilen sich auf Gespräche, Tabs, Entscheidungen und Entwürfe. ShogunAI bewahrt diesen Kontext, damit du die Geschichte nicht jedes Mal neu zusammensetzen musst.',
    highlights: [
      { title: 'Local-first als Standard', body: 'Dein Arbeitsgedächtnis bleibt standardmäßig auf deinem Gerät. Du entscheidest, wann ein verbundener Anbieter relevanten Kontext erhält.' },
      { title: 'Kontext statt Screenshots', body: 'Im Mittelpunkt steht strukturierter, durchsuchbarer Arbeitskontext statt eines Bildarchivs. Die optionale visuelle Erinnerung ist die schmale Ausnahme: aktiv nur nach Zustimmung, lokal und nach der von dir gewählten endlichen Dauer gelöscht.' },
      { title: 'Passive Erfassung', body: 'Baue während der Arbeit ein nützliches Gedächtnis auf, ohne jede Entscheidung manuell zu notieren.' },
    ],
    steps: [
      { title: 'Erfassen', body: 'ShogunAI organisiert den von dir erlaubten Kontext lokal unter macOS.' },
      { title: 'Verbinden', body: 'Momente, Personen, Projekte und Tools werden zu einem durchsuchbaren Verlauf.' },
      { title: 'Kontrollieren', body: 'Pausiere die Erfassung, wähle Dienste und lösche lokale Erinnerungen.' },
    ],
    outcomes: ['Entscheidungsgründe wiederfinden', 'Quellen schneller finden', 'Unterbrochene Arbeit fortsetzen', 'Sensiblen Kontext lokal halten'],
    faq: [
      ['Speichert ShogunAI Screenshots?', 'Standardmäßig nicht: Die Erfassung liest Text über die Accessibility-Ebene von macOS und schreibt kein Bild. Die einzige Ausnahme ist die visuelle Erinnerung, die du selbst einschaltest. Ist sie aktiv und liefert ein Fenster überhaupt keinen Text, wird ein komprimiertes Einzelbild für die von dir gewählte endliche Dauer in der verschlüsselten lokalen Datenbank gehalten und danach automatisch gelöscht. Einzelbilder verlassen deinen Mac nie, und ein Screenshot-Archiv entsteht in keinem Fall.'],
      ['Wo wird die Erinnerung gespeichert?', 'Das Gedächtnis ist local-first und bleibt standardmäßig auf deinem Mac. Optionale Dienste erhalten nur den für eine freigegebene Aktion nötigen Kontext.'],
      ['Muss ich manuell Notizen schreiben?', 'Nein. Passive Erfassung reduziert manuelle Notizen; Kontext kann bewusst ergänzt oder entfernt werden.'],
    ],
  },
