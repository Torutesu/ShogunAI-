...
const jaFeatures: readonly MarketingDetail[] = [
  {
    ...
    highlights: [
      ...
      { title: '画像ではなく文脈', body: '大量のスクリーンショットを見返す方式ではなく、検索・再利用できる構造化された仕事の文脈を中心に設計しています。任意機能のビジュアルリコールは、テキストが取れない画面をスクリーンショットで補います。オプトイン・ローカル保存で、選択した保持期間が過ぎると自動削除されます。' },
    ...
    faq: [
      ['ShogunAIはスクリーンショットを保存しますか？', '既定では保存しません。キャプチャはmacOSのアクセシビリティ経由のテキストのみで、画像は書き込みません。唯一の例外が「ビジュアルリコール」で、これは利用者が自分で有効にする機能です。有効かつウィンドウからテキストが一切取得できなかった場合にかぎり、圧縮したスクリーンショットを暗号化されたローカルDBに、選択した保持期間（プリセット1〜7日＋上限付きカスタム、既定3日）だけ保持し、期限が来ると自動削除します。保持期間中はローカルで閲覧・削除できます。フレームがMacの外へ出ることはありません。'],
...
const jaUseCases: readonly MarketingDetail[] = [
  ...
  },
  {
    ...
    highlights: [
      { title: '営業担当者', body: '今日の打ち合わせで得た価格への反応と、先月確認した技術要件をつなぎ、文脈を組み立て直さず顧客向け提案を準備します。' },
      { title: 'コンサルタント', body: 'スコープ、キックオフ時のフィードバック、過去のヒアリング記録からプロジェクト計画を作成します。' },
      { title: 'アカウントマネージャー', body: '前回の接点、顧客からの依頼、社内の担当者を、複数のシステムを掘り返さず確認します。' },
    ],
    steps: [
    ...
    outcomes: ['メール、文書、会議、メモに散らばった顧客文脈を探し回らない', 'すでに持っている文脈から提案書、フォロー、報告書を準備する', '顧客との会話に集中しながら、ShogunAIが経緯を保つ', '顧客を切り替えるたびの精神的な負担を減らす', '顧客の機密文脈を既定でローカルに保ち、共有範囲を管理する'],
    faq: [
...
const esFeatures: readonly MarketingDetail[] = [
  {
    ...
    highlights: [{ title: 'Local-first por defecto', body: 'La memoria de trabajo permanece en tu dispositivo por defecto. Tú decides cuándo un proveedor conectado recibe contexto relevante.' }, { title: 'Contexto, no capturas', body: 'Está diseñado alrededor de contexto estructurado y consultable, no de un archivo de imágenes que debas revisar. El recuerdo visual opcional añade capturas donde el texto no llega: se activa por elección, es local y se borra automáticamente al cumplir el periodo de retención que elijas.' }, { title: 'Captura pasiva', body: 'Crea una memoria útil mientras trabajas, sin convertir cada decisión en una tarea manual de notas.' }],
    ...
    faq: [['¿ShogunAI guarda capturas de pantalla?', 'No por defecto: la captura lee texto mediante la capa de accesibilidad de macOS y no escribe ninguna imagen. La única excepción es el recuerdo visual, que activas tú. Con él activo, y solo cuando una ventana no ofrece texto alguno, se guarda una captura comprimida en la base de datos cifrada local durante el periodo de retención que elijas — de uno a siete días, o una duración personalizada acotada, con tres días por defecto — y luego se borra sola al cumplir esa edad. Puedes ver o eliminar los fotogramas guardados durante ese periodo. Nunca salen de tu Mac.'], ['¿Dónde se guarda la memoria?', 'La memoria es local-first y permanece en tu Mac por defecto. Los servicios opcionales solo reciben el contexto necesario para una acción autorizada.'], ['¿Tengo que tomar notas manualmente?', 'No. La captura pasiva reduce el trabajo manual y permite añadir o eliminar contexto de forma intencional.']],
  },
  {
...
const deUseCases: readonly MarketingDetail[] = [
  ...
  { slug: 'consultants', eyebrow: 'Für Berater', title: 'Erinnere jeden Kunden, ohne alles im Kopf zu behalten', description: 'Bewahre kundenspezifischen Kontext, bereite dich schneller vor und erstelle Follow-ups auf Basis geleisteter Arbeit.', intro: 'Beratung verlangt schnelle Wechsel zwischen Unternehmen, Personen, Begriffen und Zusagen. ShogunAI stellt vor dem Gespräch den richtigen Kontext bereit und unterstützt danach die Ausarbeitung.', highlights: [{ title: 'Vertriebsprofis', body: 'Verbinde Preisfeedback aus dem heutigen Gespräch mit den technischen Anforderungen vom letzten Monat zu einem kundenfertigen Angebot, ohne den Kontext neu aufzubauen.' }, { title: 'Berater', body: 'Erstelle einen Projektplan aus Leistungsumfang, Kickoff-Feedback und Discovery-Notizen, die bereits in deinem Arbeitsverlauf liegen.' }, { title: 'Account Manager', body: 'Rufe den letzten Kundenkontakt, die offene Anfrage und die zuständige Person ab, ohne jedes System zu durchsuchen.' }], steps: [{ title: 'Kundengedächtnis aufbauen', body: 'Erfasse nötigen Kontext local-first.' }, { title: 'Vor dem Termin vorbereiten', body: 'Rufe Änderungen, Zusagen und offene Entscheidungen ab.' }, { title: 'Danach liefern', body: 'Erstelle Zusammenfassung oder Plan und prüfe ihn vor dem Versand.' }], outcomes: ['Beende die Suche nach Kundenkontext in E-Mails, Dokumenten, Meetings und Notizen', 'Erstelle Angebote, Follow-ups und Berichte aus bereits vorhandenem Kontext', 'Bleibe im Kundengespräch präsent, während ShogunAI den Verlauf bewahrt', 'Wechsle Kunden mit weniger mentalem Aufwand', 'Halte vertraulichen Kundenkontext standardmäßig lokal und kontrolliere, was geteilt wird'], faq: [['Kann ich Kundenkontexte trennen?', 'Du kontrollierst, was erfasst und welche Dienste verbunden werden.'], ['Werden Kundenmails automatisch gesendet?', 'Kundenbezogene Aktionen benötigen vor dem Versand deine Freigabe.'], ['Liegt das Gedächtnis nur in der Cloud?', 'Nein. Es ist standardmäßig local-first und teilt Kontext nur für autorisierte Funktionen.']] },
];
