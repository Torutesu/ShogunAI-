...
const esFeatures: readonly MarketingDetail[] = [
  {
    ...
    faq: [['¿ShogunAI guarda capturas de pantalla?', 'No por defecto: la captura lee texto mediante la capa de accesibilidad de macOS y no escribe ninguna imagen. La única excepción es el recuerdo visual, que activas tú. Con él activo, y solo cuando una ventana no ofrece texto alguno, se guarda un fotograma comprimido en la base de datos cifrada local durante un máximo de 72 horas y luego se borra solo. Los fotogramas nunca salen de tu Mac y en ningún caso se crea un archivo de capturas.'], ['¿Dónde se guarda la memoria?', 'La memoria es local-first y permanece en tu Mac por defecto. Los servicios opcionales solo reciben el contexto necesario para una acción autorizada.'], ['¿Tengo que tomar notas manualmente?', 'No. La captura pasiva reduce el trabajo manual y permite añadir o eliminar contexto de forma intencional.']],
  },
  {
