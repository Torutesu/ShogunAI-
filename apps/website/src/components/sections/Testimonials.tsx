...
function FeaturedVoice({ item }: { item: Item }) {
  return (
    <div className="relative overflow-hidden rounded-[34px] border border-white/60 bg-[linear-gradient(135deg,rgba(255,255,255,0.94),rgba(245,252,255,0.9))] p-7 shadow-[0_22px_70px_rgba(9,11,12,0.08)] backdrop-blur md:p-9">
      <div
        aria-hidden="true"
        className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(0,166,244,0.12),transparent_28%),radial-gradient(circle_at_bottom_left,rgba(255,255,255,0.95),transparent_35%)]"
      />
...
function VoiceCard({ item }: { item: Item }) {
  return (
    <figure className="lift relative overflow-hidden rounded-[28px] border border-border/80 bg-white/88 p-6 shadow-[0_18px_50px_rgba(9,11,12,0.06)]">
      <div
        aria-hidden="true"
        className="absolute inset-x-0 top-0 h-px bg-[linear-gradient(90deg,transparent,rgba(0,166,244,0.34),transparent)]"
      />
...
export function Testimonials({ t }: { t: Dictionary }) {
  ...
  return (
    <section id="testimonials" className="scroll-mt-20 bg-[#fffdf5] py-[clamp(56px,9vw,112px)]">
      <div className="container-x">
        <Reveal className="mx-auto mb-14 max-w-[720px] text-center">
          ...
          <h2 className="mx-auto mt-4 max-w-[12ch] text-balance font-display text-[clamp(34px,6vw,72px)] font-semibold leading-[1.02] tracking-[-0.045em]">
            {t.testimonials.title}
          </h2>
          <p className="mx-auto mt-5 max-w-[42rem] text-[17px] leading-relaxed text-muted">{t.testimonials.sub}</p>
        </Reveal>
