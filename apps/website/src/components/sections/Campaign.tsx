...
export function Campaign({ t }: { t: Dictionary }) {
  return (
    <div className="campaign-bar relative overflow-hidden border-b border-accent/25">
      ...
      <Link
        href="/rules"
        className="group container-x relative flex items-center justify-center gap-2 py-2 text-center text-xs font-medium text-ink transition-colors hover:text-accent-strong sm:py-2.5 sm:text-[13px]"
      >
        <Sparkles className="campaign-spark size-3.5 shrink-0 text-accent" />
        <span className="text-balance">{t.campaign.text}</span>
        <span className="hidden shrink-0 items-center gap-1 rounded-full bg-accent px-2.5 py-0.5 font-semibold text-white shadow-[0_2px_8px_rgba(0,166,244,0.3)] sm:inline-flex">
          {t.campaign.cta}
