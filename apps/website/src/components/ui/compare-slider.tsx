...
import { useComposedRefs } from '@/components/ui/compare-slider-utils/compose-refs';
import { cn } from '@/lib/utils';
import { useAsRef } from '@/components/ui/compare-slider-utils/use-as-ref';
import { useIsomorphicLayoutEffect } from '@/components/ui/compare-slider-utils/use-isomorphic-layout-effect';
import { useLazyRef } from '@/components/ui/compare-slider-utils/use-lazy-ref';
...
function CompareSlider(props: CompareSliderProps) {
  ...
  const store = React.useMemo<Store>(() => {
    const s: Store = {
      ...
      setState: <K extends keyof StoreState>(key: K, value: StoreState[K]) => {
        ...
        if (key === 'value') onValueChangeRef.current?.(value as number);
        s.notify();
      },
      notify: () => {
  ...
  useIsomorphicLayoutEffect(() => {
    if (valueProp !== undefined) store.setState('value', clamp(valueProp, 0, 100));
  }, [valueProp]);

  const onPointerMove = React.useCallback(
    (event: React.PointerEvent<RootElement>) => {
      if (!isDraggingRef.current && propsRef.current.interaction === 'drag') return;
      ...
      store.setState('value', clamp((position / size) * 100, 0, 100));
    },
    [propsRef, store],
  );
  ...
  const onKeyDown = React.useCallback(
    (event: React.KeyboardEvent<RootElement>) => {
      ...
      if (event.key === 'Home') {
        ...
        const isDecreaseKey = isVertical
    ...
    <StoreContext.Provider value={store}>
      <CompareSliderContext.Provider value={contextValue}>
        <RootPrimitive
          ...
          tabIndex={0}
          className={cn(
            'relative isolate touch-none select-none overflow-hidden outline-none transition-all focus-visible:ring-[3px] focus-visible:ring-accent/40',
...
function CompareSliderBefore(props: CompareSliderBeforeProps) {
  ...
  const BeforePrimitive = asChild ? SlotPrimitive : 'div';
...
function CompareSliderAfter(props: CompareSliderAfterProps) {
  ...
  const AfterPrimitive = asChild ? SlotPrimitive : 'div';
...
function CompareSliderHandle(props: DivProps) {
  ...
  const HandlePrimitive = asChild ? SlotPrimitive : 'div';
  ...
  return (
    <HandlePrimitive
      ...
      style={{ [isVertical ? 'top' : 'left']: `${value}%`, ...style }}
    >
      {children ?? (
        <>
          <div
          ...
          />
          {interaction === 'drag' && (
            <div className="z-50 flex aspect-square size-11 shrink-0 items-center justify-center rounded-full bg-surface p-2 shadow-[var(--shadow-float)] [&_svg]:size-4 [&_svg]:select-none [&_svg]:text-muted">
              {isVertical ? (
...
function CompareSliderLabel(props: CompareSliderLabelProps) {
  ...
  return (
    <LabelPrimitive
      ...
      className={cn(
        'absolute z-20 rounded-md border border-border bg-surface/80 px-3 py-1.5 text-sm font-medium backdrop-blur-sm',
        isVertical
          ? side === 'before'
            ? 'top-2 left-2'
            : 'bottom-2 left-2'
