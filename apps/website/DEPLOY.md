...
| `SUPPORT_NOTIFY_TO` / `SUPPORT_NOTIFY_FROM` | optional | Override the notification inbox and envelope sender. Both default to `info@shogunaios.com`. `FROM` must be on a domain verified in Resend, or every send is rejected. |
