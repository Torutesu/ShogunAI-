...
**既存フレームの完全な再スキン**であり、レイアウトは1pxも動かさない:
10.1 = 3.3（expanded welcome, 560×360）/ 10.2 = 3.4（context actions）/ 10.3 = 4.2（Morning brief）。
変えたのはマテリアルだけ — 不透明 `#12151C` ガラス → デフォーカスした実写新緑の上の
ティントガラス（background blur 80、白10%＋深緑14%、スペキュラエッジ）。
チップ・コンポーザー・L1/L2/L3タグ・出典チップは同位置・同サイズ・同文言
（L色のみ緑ガラス上のコントラスト確保で1段暖色: `#34C759` / `#8FD3FF` / `#FFD60A`）。
シェルは `Mac / Desktop 14″ Ambient`。

**会議面（10.4〜10.8、2026-08-17 追加）**: 5.1 検知オファー / 5.2 録音カプセル / 5.3 AI Canvas /
