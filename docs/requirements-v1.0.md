...
| 2 | 画像・音声はデバイスから出さず期限つきでのみ保存（2026-07-30改定。連続録画・クラウドVLM/ASR禁止） | FR-CAP-01、FR-VIS群（§6.21）、NFR-PRV-01、FR-MT群 |
...
| 視覚キャプチャ | イベント駆動キーフレーム＋オンデバイスOCR（既定7日保持・暗号化。検証スパイク合格が本実装の前提。§6.21） |
| 会議音声の保存 | オプトイン下のローカル暗号化保存（既定30日・自動削除。FR-MT-12改定） |
| 会議ノートUX | 専用ノートウィンドウ、co-writing清書、会議ライブラリ、リアルタイム翻訳（§6.22） |
...
| 項目 | 予定 | v1で断る根拠 |
|---|---|---|
| 動画の連続録画・録画ファイルの生成 | **恒久的に対象外** | 不変条件2（2026-07-30改定後も維持）。画面から取得するのはイベント駆動のキーフレームのみ（§6.21）。会議音声のローカル暗号化保存はv1スコープ入り（FR-MT-12改定） |
| 画像・音声のクラウド送信（クラウドVLM / クラウドASR） | **恒久的に対象外** | 不変条件3。ローカル処理の精度が不足しても例外を作らない（OPEN-17決定） |
...
| MT3 | オンデバイスASR + `transcript_segments` + 暗号化音声保存・保持期限（FR-MT-12改定） | あり | MT2 + OPEN-07/08 |
...
**FR-CAP-01（MUST, All）〔2026-07-30改定〕**: キャプチャの主経路は **Accessibility API（AXUIElement）経由のテキスト**。これに視覚キャプチャ（イベント駆動キーフレーム＋オンデバイスOCR、§6.21）を併設できるが、**連続録画・動画を取得・保存するコードは書かない**（CLAUDE.md不変条件2）。会議音声は本節のキャプチャレーンとは**別レーン**であり、FR-MT群（§6.16）の条件下（Recording中のみ・RAM内処理・テキストのみ永続化）でのみ扱う。**常時キャプチャの一部として音声を取得しない。**取得対象: フォーカス中ウィンドウのアプリ名（bundle id）・ウィンドウタイトル・可視テキスト（AX階層から抽出）・フォーカス要素のロール、およびNSWorkspaceからのアプリ切替イベント。
...
**FR-AG-06（MUST, Pro）**: エージェント推論・チャット・ドラフト生成は**ユーザーBYOK（Anthropic）**で実行する。Select KKキーをこの用途に使わない（CLAUDE.md不変条件5）。BYOK未設定のProユーザーには該当機能の起動時に設定導線を出す。
...
- **Pro**のエージェント推論・チャット・ドラフト生成は**BYOK（ユーザーのAnthropicキー）必須**。Select KKキーをエージェント推論に使わない
- この対応により「プラン境界＝キー境界」となり、逆転（StandardでBYOK要求、ProでSelect KKキーによる推論）を実装しない
...
**FR-BIL-04（MUST）**: 初回起動から7日間、Pro相当の全機能を提供する。トライアル中のBYOK: エージェント機能を試すにはBYOK設定が必要（Select KKキーで肩代わりしない。不変条件5維持）。BYOK未設定のトライアルユーザーにはStandard相当機能＋エージェント機能のBYOK設定導線を提示する。
...
| 2 | Accessibility＋画面収録権限 | 目的説明（何を抽出し、何がデバイス外に出ないか・キーフレームは既定7日で自動削除）→システム設定へ誘導→付与検出で次へ。画面収録の拒否時はAXテキストのみへ縮退（FR-VIS-07） |
| 3 | キャプチャ開始 | 付与確認後、キャプチャを開始しNotchデモ（Hover→Expandedのガイド）を実施 |
| 4 | 連携接続 | Wave 1（Gmail / Google Calendar）の接続を提案。スキップ可 |
| 5 | （Pro/トライアル）BYOK設定 | Anthropicキーの入力（Keychain保存）。スキップ可（エージェント機能ロックのまま進む） |
...
**FR-OB-02（MUST, All）〔2026-07-30改定〕**: ステップ2の説明画面には次を明記する: キャプチャはAXテキスト＋イベント駆動キーフレーム（連続録画はしない）/ 画像・音声はデバイス外に出ない（クラウド送信なし）/ キーフレームは既定7日で自動削除・いつでも削除できる / 除外リストと一時停止がいつでも使える。

**FR-OB-06（MUST, All）**: ステップ6（会議ノート）でONを選んだ場合のみ、マイク／音声取得のTCC権限を要求する。**OFFのまま権限を要求しない。**（画面収録権限はステップ2の視覚キャプチャ用であり、本ステップとは独立に扱う） 説明にはFR-MT-03の開示テキストを用い、「ShogunAIはZoom / Google Meetにbot参加しません」「承認した会議だけを聞き、音声はこのMacの外に出ません（既定30日で自動削除・いつでも削除可）」を明記する。
...
| **Capture** | キャプチャON/OFF / 視覚キャプチャON/OFF・保持モード（期間保持/即破棄）・保持期間（既定7日）・ストレージ使用量（FR-VIS群）/ 除外アプリリスト（既定項目表示・追加/削除、FR-CAP-06の削除不可項目はグレーアウト）/ 除外ドメイン・タイトルパターン / 一時停止の時限オプション既定値 |
| **Meeting notes** | 機能ON/OFF（**既定OFF**、FR-MT-01）/ 会議アプリ別の除外リスト（FR-MT-02(b)）/ 常に除外する予定（繰り返し1on1等）/ 音声保持期間（既定30日）・音声ストレージ使用量・全削除（FR-MT-12）/ ライブ文字起こしトグル既定・翻訳言語（FR-MTUX-05/06）/ 開示テキストの常設表示（FR-MT-03）/ 直近セッション一覧とRecap・会議ライブラリへの導線 |
...
> **本節を貫く原則〔2026-07-30改定〕**: 「**聞いて、外に出さない**」。音声はこのMacの中だけで処理・保存され（暗号化・期限つき・いつでも削除可）、デバイス外へは一切出ない（不変条件2改定・不変条件3、NFR-PRV-01）。
...
**FR-MT-03（MUST, All）〔2026-07-30改定〕**: 開示テキストをRecap画面と設定画面に常設する: 「この会議の音声はこのMacの中だけで処理・保存され、外部には送信されません。録音は{保持期間}日で自動削除され、いつでも削除できます」。**事実と実装が一致していること**が受け入れ条件（実装がこの文言を裏切っていないことをテストで担保）。法域別の分岐は持たない（既定OFF＋明示オプトイン＋常時可視のピルで一律運用）。参加者への告知を**プロダクトが代行しない**（会議へ自動でメッセージ・チャット投稿をしない。CLAUDE.md不変条件4／Issue #7 Non Goal）。
...
**FR-MT-12（MUST, All）〔2026-07-30改定: リングバッファ30秒制限を撤廃し、ローカル暗号化保存を許可〕**: 音声の経路は以下に限る。**デバイスの外に音声を出さない。**

```
 マイク ────┐                        ┌─→ オンデバイスASR ─→ transcript_segments（テキスト）
            ├─→ [取得バッファ(RAM)] ─┤
 システム音声┘                        └─→ セッション音声ファイル（ローカル暗号化・既定30日で自動削除）
 (Core Audio tap)                          └→ transcript_segmentsと時刻リンク（聞き直し・再文字起こし用）
```

- 音声の取得は**Recording状態の間のみ**。Idle/Offered/Wrapping中は音声デバイスにアクセスしない
- 音声ファイルは**セッション単位・ローカル暗号化**（鍵はKeychain）。保持期間は既定30日（OPEN-16決定。設定変更可）、期限超過で自動削除、セッション単位の即時削除可
- 音声は**いかなる経路でもデバイス外へ送信しない**（クラウドASR禁止。不変条件3・OPEN-17決定）
- 保存の目的は検証可能性（Recapからの該当箇所の聞き直し）と精度改善（ASRモデル更新時の再文字起こし）であり、無期限アーカイブではない

**FR-MT-13（MUST, All）〔2026-07-30改定〕**: ASRは**オンデバイス**とし、音声をクラウドに送信しない。ストリーミング・ファイルベースのいずれも選定可（ローカル保存の許可に伴い「ファイル入力を要求する実装は選ばない」制約を撤廃）。モデルは同梱または初回DLとし、選定はサイズ・精度と会議中CPU（FR-MT-21）のトレードオフで決める（§9 未決事項）。macOS 14.4未満でCore Audio tapが使えない環境での縮退挙動（マイクのみに縮退するか機能を出さないか）を明示的に決めて実装すること（§9）。
...
**受け入れ基準（6.16）**:
1. **音声保存の規律検査〔2026-07-30改定〕**: Recording中に生成される音声ファイルが暗号化されていること、Recording外で音声デバイスにアクセスしないこと、保持期限超過で自動削除されること、即時削除がファイル・参照の全てを消すこと、音声データが外部送信経路に到達しないことを検証する（FR-MT-12、NFR-PRV-01）。
...
### 6.21 視覚キャプチャ（FR-VIS群）〔2026-07-30 不変条件2改定に伴う新設〕

**目的**: AXテキストの空白地帯（Electron・独自描画・図表・画像内テキスト）を埋め、ワールドモデルの精度を上げる。**連続録画はしない。画像はデバイスから出さない。**

**FR-VIS-01（MUST, All）**: 視覚キャプチャはScreenCaptureKitによる**イベント駆動のキーフレーム取得のみ**とし、連続録画（動画）を行わない。取得トリガ: フォーカス切替・有意な内容変化（AXテキスト差分・スクロール静止後）。perceptual hashで重複フレームを破棄する。

**FR-VIS-02（MUST, All）**: 取得フレームは即時**オンデバイス処理**（Vision framework OCR。将来ローカルVLMへの差し替え余地を持つ）でテキスト・構造を抽出し、既存のevent log経路に乗せる（専用経路を作らない。ソース対称の原則）。

**FR-VIS-03（MUST, All）**: フレーム保持の既定は**期間保持モード**（ダウンスケール・再圧縮したキーフレームを暗号化して保持。既定7日・期限超過で自動削除。OPEN-15決定）。設定で「抽出後即破棄」への変更・保持期間の変更・即時全削除が可能。

**FR-VIS-04（MUST, All）**: 除外リスト（FR-CAP-05/06）・一時停止（FR-CAP-07）・SecureTextField除外を視覚キャプチャに**同一適用**する。除外対象がフォーカス中はフレーム取得自体を行わない。NFR-SEC-07のsecretリダクションを抽出テキストに適用する。

**FR-VIS-05（MUST, All）**: 画像は**いかなる経路でもデバイス外へ送信しない**（クラウドVLM禁止。不変条件3・OPEN-17決定）。Batch API / BYOK / MCPへ送られるのは抽出テキストのチャンクのみ。保持フレームのストレージ使用量を設定画面に表示する。

**FR-VIS-06（MUST, All）**: リソースは**別枠予算**で計測する（視覚キャプチャ＋OCRのCPU・メモリ・ストレージ。アイドルSLO 5%と混ぜない）。ストレージ上限到達で古い順に自動削除。バッテリー影響を計測しContext Healthに表示する。

**FR-VIS-07（MUST, All）**: 視覚キャプチャは**画面収録（Screen Recording）TCC権限**を要する。方針は既定ON（OPEN-14決定）とし、オンボーディングで目的（何を抽出し、何がデバイス外に出ないか・保持期限）を開示した上で権限を要求する。拒否時はAXテキストのみの動作へグレースフルに縮退する（FR-CAP-08と同型。付与の再検出・自動有効化も同様）。

**検証ゲート（MUST）**: 本実装前にスパイクで次に答える: (1) イベント駆動取得＋OCRのCPU/バッテリー実測が別枠予算内か (2) OCR追加でstate抽出・検索の精度が実際にどれだけ上がるか（評価セットで前後比較） (3) ストレージ増加率の実測。No-Goの場合は「抽出後即破棄モードのみ」へ縮退して出荷する。

**受け入れ基準（6.21）**: 連続録画APIを呼ぶコードが存在しないことのアーキテクチャテスト。除外アプリ・一時停止中にフレームが1枚も生成されないテスト。保持期限超過の自動削除テスト。画像データが外部送信経路（共通HTTP出口）に到達しないテスト。権限拒否時の縮退テスト。

### 6.22 会議ノートUX（FR-MTUX群）〔2026-07-30 新設〕

**目的**: FR-MT群（仕組み: 検知・ASR・Recap・state接地）の上に「ノートプロダクトとしての完成度」を載せる。SHOGUNの規律（割り込まない・根拠付き・state接地）は維持する。
...
**FR-MTUX-02（MUST, All）**: Full UIに**会議ライブラリ**: 検索（既存FTS）、人物・プロジェクト別フィルタ、繰り返し会議のシリーズビュー（前回の決定・宿題を次回に引き継ぐ）、各会議のRecap / 文字起こし / ノート / 保存音声（FR-MT-12）への導線。

**FR-MTUX-03（MUST, All）**: **co-writing**: 会議中のユーザーメモは断片で良い前提とし、会議後にAIが文字起こしと突合して清書（enhance）する。清書はユーザーのメモを**上書きせず**、原文＋清書版の2層で保存する（いつでも原文に戻れる）。清書はRecap生成（FR-MT-16、Select KK Batch）と同一ジョブで行う。

**FR-MTUX-04（SHOULD, All）**: 会議中、決定・宿題らしき発言を検知したら、ノート欄の**専用レーンに候補チップを静かに追加**する（無視・削除できる。音・ポップアップなし）。ライブ文字起こし全文表示とは別物であり、FR-MT-10の趣旨を壊さない。

**FR-MTUX-05（MUST, All）**: FR-MT-10を改定し、ライブ文字起こしペインを**既定OFFのトグル**として提供する（既定挙動は現行どおり非表示）。

**FR-MTUX-06（MUST, All）**: **リアルタイム翻訳**: ASRテキストを設定言語へストリーミング翻訳して表示するペインを提供する。方式はmacOS Translation frameworkを第一候補にスパイクで確定し（OPEN-18決定）、品質不足時のみBYOKテキスト翻訳をオプトインで併設する（音声は出さない・トレーサビリティ必須）。CPUは会議中15%枠（FR-MT-21）に含め、超過時は翻訳から間引く（OPEN-19決定）。
...
**FR-MTUX-09（MUST, All）**: エクスポート: Markdownコピー / ローカルファイル書き出し = L2。外部サービスへの共有・送信はL3で、v1スコープ外（§3.2踏襲）。

**受け入れ基準（6.22）**: 清書がユーザー原文を書き換えないテスト。ライブ文字起こしトグルの既定OFFテスト。翻訳経路に音声データが乗らないテスト。翻訳ONでの会議中CPUが15%枠に収まる計測。
...
**NFR-PRV-01（MUST）〔2026-07-30改定〕**: 画像・音声は**デバイスから出さず、期限つきでのみ保存する**（不変条件2改定）。連続録画・動画を生成するコードを書かない。視覚キャプチャはイベント駆動キーフレームのみ（暗号化・既定7日で自動削除、FR-VIS群）。会議音声の取得はRecording中に限り、ローカル暗号化保存（既定30日・自動削除・即時削除可、FR-MT-12）。画像・音声はいかなる経路でもデバイス外へ送信しない（クラウドVLM / クラウドASR禁止）。文字起こしテキストとそのprovenance（`origin`: asr / caption）は従来どおり永続化する。
...
| NFR-RES-03 | ディスク | Warm+Cold+索引の合計使用量を設定画面に表示。20GB到達で通知（アンバー）し、Cold層の圧縮・削除導線を提示。キーフレーム（FR-VIS-03）・会議音声（FR-MT-12）は**別枠上限**（各5GB既定・古い順自動削除）で層別表示 |
...
| OPEN-07 | オンデバイスASRモデルの選定（オンデバイス・多言語） | MT3着手前のベンチ | 要件はFR-MT-13で固定。モデルサイズ・精度と会議中CPU（FR-MT-21の15%枠）のトレードオフで判断。ストリーミング・ファイルベースのいずれも選定可（FR-MT-12改定 2026-07-30） |
...
| OPEN-14 | 視覚キャプチャの既定 | **既定ON＋オンボーディングで明示開示**（TCC権限要求が実質の同意。FR-VIS-07） |
| OPEN-15 | フレーム保持の既定 | **期間保持モード・既定7日**（FR-VIS-03。即破棄モードへ変更可） |
| OPEN-16 | 会議音声の既定保持期間 | **30日**（Warm層と同期。FR-MT-12。自動削除・即時削除可） |
| OPEN-17 | 画像・音声のデバイス外送信 | **不変条件として固定**（クラウドVLM / クラウドASRの例外を将来も作らない。不変条件3に明文化） |
| OPEN-18 | リアルタイム翻訳の方式 | **macOS Translation frameworkを第一候補にスパイク**。品質不足時のみBYOKテキスト翻訳をオプトイン併設（FR-MTUX-06） |
| OPEN-19 | ライブ表示・翻訳のCPU予算 | **会議中15%枠（FR-MT-21）に含める**。超過時は翻訳から間引く |
...
| Accessibility APIで十分なテキストが取れないアプリ（Electron・独自描画） | キャプチャ空白地帯 | 取得可否のアプリ別実測をPhase 1早期に実施。取れないアプリは**視覚キャプチャ（FR-VIS群: キーフレーム＋オンデバイスOCR）**と連携（第1層）データで補完（2026-07-30改定。連続録画への転換はしない） |
