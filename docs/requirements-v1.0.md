...
| 2 | 画像・音声ファイルを保存しない（音声はRAM処理・テキストのみ永続化。画像は Visual recall の明示的例外のみ） | FR-CAP-01、NFR-PRV-01、FR-MT群、FR-PTT群（§6.17） |
...
| **Visual recall** | AXでテキストが取れないウィンドウのみ、圧縮JPEGを暗号化DB（`screen_frames`）へ保持しオンデバイスOCRで読む任意機能。**既定Off**、保持期間はユーザー選択、期限切れは自動削除。§6.2 / FR-CAP-01 |
| **音声入力（push-to-talk）** | ショートカット長押し中のみマイクを開き、離すと文字起こしがキャレット位置へ着地する入力経路。音声はディスク・DBへ書かない。§6.17 |
| **Scribe** | 任意アプリの編集可能フィールドを選択し、指示どおりその場で書き換える機能。捕捉→再検証→保護スパン検証を経てのみ書き込む。§6.18 |
...
| 領域 | v1に含む |
|---|---|
| キャプチャ | Accessibility API経由のテキストキャプチャ、除外リスト、一時停止、Visual recall（既定Off・端末内暗号化・保持期間はユーザー選択・自動削除） |
...
| 入力（**【2026-08-20 実装反映】**） | **音声入力（push-to-talk）**: ショートカット長押し中のみマイク、離すと文字起こし＋文脈結合（§6.17）／**Scribe**: 任意アプリの編集欄をその場で書き換え（§6.18） |
...
**FR-CAP-01（MUST, All）**: キャプチャは **Accessibility API（AXUIElement）経由のテキストのみ**。スクリーンショット・画像・動画・OCRを取得・保存するコードを書かない（CLAUDE.md不変条件2）。**【2026-08-02 明示的例外｜Visual recall / **【2026-08-20 実装反映】** 保持期間を実装に合わせて更新】** Visual recall が On のときにかぎり、AX からテキストが取得できなかったウィンドウについて圧縮 JPEG を暗号化済みメモリ DB（`screen_frames`）に保持し、期限切れは自動削除する。既定は Off、クラウド送信なし、音声は対象外、永続タイムラインではない（CLAUDE.md 不変条件2の例外）。**保持期間はユーザーが選ぶ**（既定3日／プリセット1〜7日／カスタム上限3,650日。`crates/shogun-core/src/capture/visual_recall.rs`）。⚠ CLAUDE.md 2026-08-02 例外の原文は「最大72時間」だが、実装のカスタム上限はそれを超える。上限を戻す場合は `MAX_CUSTOM_RETENTION_DAYS` の変更だけで済む（判断は `docs/spec-implementation-drift-audit.md` §2-C）。 会議音声は本節のキャプチャレーンとは**別レーン**であり、FR-MT群（§6.16）の条件下（Recording中のみ・RAM内処理・テキストのみ永続化）でのみ扱う。**常時キャプチャの一部として音声を取得しない。**取得対象: フォーカス中ウィンドウのアプリ名（bundle id）・ウィンドウタイトル・可視テキスト（AX階層から抽出）・フォーカス要素のロール、およびNSWorkspaceからのアプリ切替イベント。
...
**FR-OB-02（MUST, All）**: ステップ2の説明画面には次を明記する: キャプチャはAccessibility API経由のテキストのみ / 既定ではスクリーンショット・画像を保存しない（Visual recall を自分で On にした場合のみ、テキストが取れなかったウィンドウのフレームを**ユーザーが選んだ保持期間**でローカル保持し、期限切れは自動削除することを併記する。既定の保持期間もその場に表示する） / 生データはデバイス外に出ない / 除外リストと一時停止がいつでも使える。
...
### 6.17 音声入力 / Push-to-Talk（**【2026-08-20 実装反映】** 実装済み機能の事後要件化）

**目的**: 作業フローを止めずにSHOGUNへ話しかける入口を用意する。押している間だけ話し、離した瞬間にテキストが着地する。設計は `docs/push-to-talk-voice-design.md`（Issue #44）、実装は `apps/desktop/src-tauri/src/{voice_shortcut,voice_session,voice_lane}.rs` と `VoiceOverlay.tsx`。

> 本節は**新規要件ではなく、先行実装された機能の要件化**である。以下は実装の現在の挙動を受け入れ基準の粒度で記述したもので、実装と食い違う記述を見つけた場合は `docs/spec-implementation-drift-audit.md` に追記する。

**FR-PTT-01（MUST, All）**: 起動は**ショートカットの長押し（hold）**とする。押下中のみマイクを開き、離した時点で入力を終える。押しっぱなしを要求するのは誤起動と「録りっぱなし」を構造的に防ぐため。トグル方式を既定にしない。組み合わせは `shortcuts.json` の `voice` アクションとして永続化し、変更を即時反映する。

**FR-PTT-02（MUST, All）**: **音声をディスク・DBのいずれにも書かない。** 一時ファイルも作らない。波形はメモリ上のストリーム/リングバッファとしてのみ扱い、永続化するのは文字起こしテキストのみ（CLAUDE.md 不変条件2）。

**FR-PTT-03（MUST, All）**: 文字起こしの既定経路は**クラウドのライブ STT**（Deepgram live WS）とし、ハンドシェイク失敗時は HTTP バッチ、さらにオンデバイス（Whisper）へ縮退する。**縮退を黙って行わない**（UIに経路を示す）。オンデバイスモデルは NSEvent / AppKit スレッドで読み込まない（UIを固めるため）。

**FR-PTT-04（MUST, All）**: **音声 egress のトレーサビリティを必須**とする。記録できない状態（メモリDB未初期化等）では**音声入力を実行しない**（実装では `purpose = voice_dictation` のシンクが取得できなければエラーで停止する）。記録内容はダイジェストとバイト長のみで、発話内容そのものを残さない。

**FR-PTT-05（MUST, All）**: **UI開示**: 設定の音声セクションと初回利用時に、(a) ライブ文字起こしが外部の音声認識を通ること、(b) 学習利用はオプトアウトされていること、(c) 音声はディスクに書かないこと、(d) オフライン時はオンデバイスへ縮退すること、を明示する。
> ⚠ CLAUDE.md 2026-08-05 の明示的例外は文面上「Meeting notes の既定 ASR」に限定されている。**音声入力もクラウド STT を通る**ため、例外の文言を会議 ASR と音声入力の双方を含む形へ広げる必要がある（判断は `docs/spec-implementation-drift-audit.md` §2-D）。

**FR-PTT-06（MUST, All）**: 出力先は**離した時点でフォーカスしていた編集可能フィールドのキャレット位置**とする。挿入対象が取得できない・書き込み不可の場合はクリップボードへ退避し、その旨をUIに出す。**黙って消さない。**

**FR-PTT-07（MUST, All）**: 文脈結合: 文字起こしは、プリアセンブル済みの context cache（§5.4）を参照して人名・プロジェクト名などの表記を補正できる。**補正は表記に限り**、発話にない内容を足さない。

**FR-PTT-08（MUST, All）**: 状態機械の安全性: 押下と解放の処理は単一ワーカーで直列化し、素早いタップで解放が押下より先に処理される（＝録音状態が固着する）ことを構造的に防ぐ。マイク使用中はインジケータを表示し、解放後は必ず Idle へ戻る。

**受け入れ基準（6.17）**: 押下→解放→着地のE2E。音声がディスクへ書かれないことの検査（ファイルシステム監視）。トレーサビリティ・シンクが無い環境で実行が停止することのテスト。挿入不可アプリでクリップボード退避に落ちるテスト。高速タップで状態が固着しないテスト。

### 6.18 Scribe（その場書き換え）（**【2026-08-20 実装反映】** 実装済み機能の事後要件化）

**目的**: どのアプリの文章でも、選択して指示するだけで**その場で**書き換える。コピー・ペースト・別ウィンドウを不要にする。実装は `apps/desktop/src-tauri/src/scribe.rs`（macOSアダプタ）と `crates/shogun-core/src/inline.rs`（プロンプト構築と検証）、UI は `ScribeOverlay.tsx`。

**FR-SCR-01（MUST, All）**: 起動時に、**オーバーレイがフォーカスを奪う前**に編集対象（AX要素・現在値・選択レンジ）を捕捉する。捕捉できなければ起動しない。

**FR-SCR-02（MUST, All）**: 書き込みは、**捕捉時と同一のターゲット・同一の値・同一の UTF-16 レンジ**であることを再検証してからのみ行う。ひとつでも変化していれば書き込まず、失敗として通知する（別の文章を破壊しないため）。

**FR-SCR-03（MUST, All）**: **保護スパンの保存検証**: 固有名詞・識別子・数値など、指示で明示的に変更を求められていない要素が書き換わっていないことを検証する。検証に落ちた出力は**破棄し、原文を維持する**（`scribe_output_preserves_protected_spans`）。

**FR-SCR-04（MUST, All）**: **画面から取り込んだテキストは信頼しない**。モデルへは system / user を分離して渡し、捕捉したコンテキストは untrusted なデータとして扱う（プロンプトインジェクション耐性。`build_scribe_edit_split_prompt`）。

**FR-SCR-05（MUST, All）**: 状態を UI に返す: `opened` / `processing` / `inserted` / `failed` / `cancelled` / `closed` / `no_key`。**資格情報が無い場合（`no_key`）は明示状態として返す**——無反応で終わらせない。

**FR-SCR-06（MUST, All）**: 書き換えはローカル編集であり L1 相当（可逆・外部送信なし）。ただし推論のために選択テキストが外部モデルへ出るため、**egress のトレーサビリティ対象**とする。外部への送信（メール等）を Scribe から行わない。

**FR-SCR-07（SHOULD, All）**: キャンセル（Esc / オーバーレイを閉じる）は、書き込みが確定する前であれば必ず原文を維持したまま終了する。

**受け入れ基準（6.18）**: 捕捉後にユーザーがフォーカスを移した場合に書き込まないテスト。保護スパンが変化した出力を破棄するテスト（既存の `inline.rs` テスト群）。`no_key` がUIへ届くテスト。キャンセル時に原文が保持されるテスト。

> **プランの扱い（未決）**: 音声入力・Scribe とも、現状のコードに**プランゲートは無い**。Standard/Pro のどちらに置くか（推論を伴うため Pro 相当とするか）は未決事項として §9.1 に追加した。
...
**NFR-PRV-01（MUST）**: スクリーンショット・画像・**音声ファイルを保存**するコードを書かない（不変条件2）。**【2026-08-02 明示的例外｜Visual recall / **【2026-08-20 実装反映】** 保持期間を実装に合わせて更新】** Visual recall が On のときにかぎり、AX からテキストが取得できなかったウィンドウについて圧縮 JPEG を暗号化済みメモリ DB（`screen_frames`）に保持し、期限切れは自動削除する。既定は Off、クラウド送信なし、音声は対象外、永続タイムラインではない（CLAUDE.md 不変条件2の例外）。**保持期間はユーザーが選ぶ**（既定3日／プリセット1〜7日／カスタム上限3,650日。`crates/shogun-core/src/capture/visual_recall.rs`）。⚠ CLAUDE.md 2026-08-02 例外の原文は「最大72時間」だが、実装のカスタム上限はそれを超える。上限を戻す場合は `MAX_CUSTOM_RETENTION_DAYS` の変更だけで済む（判断は `docs/spec-implementation-drift-audit.md` §2-C）。 音声ファイルについては例外なし（ディスクへ書かない点は音声入力・会議とも共通。§6.17 / §6.16）。会議音声の取得は会議セッション中に限り、波形はRAM内リングバッファ（上限30秒）でのみ処理し、ディスク・一時ファイル・クラウドのいずれにも書かない。永続化するのは文字起こしテキストとそのprovenance（`origin`: asr / caption）のみ。詳細はFR-MT群（`docs/meeting-notes-ui-design.md`）。
...
| OPEN-10 | **Visual recall の保持上限**（**【2026-08-20 追加】**） | 外向け説明を出す前 | CLAUDE.md 2026-08-02 例外の原文は「最大72時間」。実装は既定3日・カスタム上限3,650日。文書を実装へ合わせるか、`MAX_CUSTOM_RETENTION_DAYS` を絞るかを決める。**決まるまで外向けに「72時間で消える」と書かない**（`docs/spec-implementation-drift-audit.md` §2-C） |
| OPEN-11 | **クラウドASR例外の適用範囲**（**【2026-08-20 追加】**） | 同上 | CLAUDE.md 2026-08-05 例外は「Meeting notes の既定ASR」限定の文面だが、音声入力（FR-PTT-03）も同じ経路を使う。例外文を双方を含む形へ広げる（同 §2-D）。ディスク非書き込み・`mip_opt_out`・トレーサビリティは両者で共通 |
| OPEN-12 | **音声入力 / Scribe のプラン帰属**（**【2026-08-20 追加】**） | 課金ハードゲート実装時 | 現状コードにプランゲートなし。推論を伴うため Pro 相当とするか、入力体験として全プランに開くかを決める（ADR-003のキー境界と整合させる） |
...
| Accessibility APIで十分なテキストが取れないアプリ（Electron・独自描画） | キャプチャ空白地帯 | 取得可否のアプリ別実測をPhase 1早期に実施。取れないアプリは連携（第1層）データで補完。常時キャプチャをスクショOCRへ転換することは**しない**（不変条件2）。テキストが取れないウィンドウに限っては、Off が既定の Visual recall（2026-08-02 例外、ユーザーが選んだ期間で自動削除）が任意の補完手段になる |
