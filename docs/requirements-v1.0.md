...
**FR-MT-13（MUST, All）**: ASRは**オンデバイス・ストリーミング**とし、音声をクラウドに送信しない。モデルは同梱または初回DLとし、選定はサイズと会議中CPU（FR-MT-21）のトレードオフで決める（§9 未決事項）。macOS 14.4未満でCore Audio tapが使えない環境での縮退挙動（マイクのみに縮退するか機能を出さないか）を明示的に決めて実装すること（§9）。
