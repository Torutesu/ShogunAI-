...
**FR-AG-06a（MUST）**: サブスク委譲は Agent lane 専用とする。Batch lane（インデックス・分類・Dream Cycle・Morning Brief）には使わない。プラン枠は時間窓制限があり、バッチ量を流すとユーザー本人のCLIが使用不能になるため（不変条件5の型レベル分離で担保: `SubscriptionAgentClient` は `AgentClient` のみ実装し `BatchClient` を実装しない）。
...
**FR-AG-06e（MUST）**: プラン枠の上限到達は、資格情報の問題（401）とも SHOGUN の障害とも区別して提示する。文言はどのプランの上限かを明示し、SHOGUN側の失敗と読めてはならない。BYOKが設定済みならフォールバック導線を出す。
