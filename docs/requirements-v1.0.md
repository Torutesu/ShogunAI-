...
| 2 | スクショ・画像を保存しない | FR-CAP-01、NFR-PRV-01 |
...
| 項目 | 予定 | v1で断る根拠 |
|---|---|---|
| 音声キャプチャ・会議文字起こし・会議サマリ | **v1.5** | キャプチャはAccessibility APIテキストのみ（CLAUDE.md不変条件2の範囲設計） |
...
**FR-CAP-01（MUST, All）**: キャプチャは **Accessibility API（AXUIElement）経由のテキストのみ**。スクリーンショット・画像・動画・OCRを取得・保存するコードを書かない（CLAUDE.md不変条件2）。取得対象: フォーカス中ウィンドウのアプリ名（bundle id）・ウィンドウタイトル・可視テキスト（AX階層から抽出）・フォーカス要素のロール、およびNSWorkspaceからのアプリ切替イベント。
...
| 6 | 完了 | Morning Briefの予告（「明日の朝、ここに現れる」）と一時停止ホットキーの案内 |
...
**NFR-PRV-01（MUST）**: スクリーンショット・画像・音声を取得・保存するコードを書かない（不変条件2。音声はv1.5で別途要件化）。
