...
| 2 | 画像・音声ファイルを保存しない（音声はRAM処理・テキストのみ永続化。画像は Visual recall の明示的例外のみ） | FR-CAP-01、NFR-PRV-01、FR-MT群、FR-PTT群（§6.22） |
...
| **音声入力（push-to-talk）** | ショートカット長押し中のみマイクを開き、離すと文字起こしがキャレット位置へ着地する入力経路。音声はディスク・DBへ書かない。§6.22 |
| **Scribe** | 任意アプリの編集可能フィールドを選択し、指示どおりその場で書き換える機能。捕捉→再検証→保護スパン検証を経てのみ書き込む。§6.23 |
...
| 入力（**【2026-08-20 実装反映】**） | **音声入力（push-to-talk）**: ショートカット長押し中のみマイク、離すと文字起こし＋文脈結合（§6.22）／**Scribe**: 任意アプリの編集欄をその場で書き換え（§6.23） |
...
### 6.22 音声入力 / Push-to-Talk（**【2026-08-20 実装反映】** 実装済み機能の事後要件化）
...
**受け入れ基準（6.22）**: 押下→解放→着地のE2E。音声がディスクへ書かれないことの検査（ファイルシステム監視）。トレーサビリティ・シンクが無い環境で実行が停止することのテスト。挿入不可アプリでクリップボード退避に落ちるテスト。高速タップで状態が固着しないテスト。

### 6.23 Scribe（その場書き換え）（**【2026-08-20 実装反映】** 実装済み機能の事後要件化）
...
**受け入れ基準（6.23）**: 捕捉後にユーザーがフォーカスを移した場合に書き込まないテスト。保護スパンが変化した出力を破棄するテスト（既存の `inline.rs` テスト群）。`no_key` がUIへ届くテスト。キャンセル時に原文が保持されるテスト。
...
**NFR-PRV-01（MUST）**: スクリーンショット・画像・**音声ファイルを保存**するコードを書かない（不変条件2）。**【2026-08-02 明示的例外｜Visual recall / **【2026-08-20 実装反映】** 保持期間を実装に合わせて更新】** Visual recall が On のときにかぎり、AX からテキストが取得できなかったウィンドウについて圧縮 JPEG を暗号化済みメモリ DB（`screen_frames`）に保持し、期限切れは自動削除する。既定は Off、クラウド送信なし、音声は対象外、永続タイムラインではない（CLAUDE.md 不変条件2の例外）。**保持期間はユーザーが選ぶ**（既定3日／プリセット1〜7日／カスタム上限3,650日。`crates/shogun-core/src/capture/visual_recall.rs`）。⚠ CLAUDE.md 2026-08-02 例外の原文は「最大72時間」だが、実装のカスタム上限はそれを超える。上限を戻す場合は `MAX_CUSTOM_RETENTION_DAYS` の変更だけで済む（判断は `docs/spec-implementation-drift-audit.md` §2-C）。 音声ファイルについては例外なし（ディスクへ書かない点は音声入力・会議とも共通。§6.22 / §6.16）。会議音声の取得は会議セッション中に限り、波形はRAM内リングバッファ（上限30秒）でのみ処理し、ディスク・一時ファイル・クラウドのいずれにも書かない。永続化するのは文字起こしテキストとそのprovenance（`origin`: asr / caption）のみ。詳細はFR-MT群（`docs/meeting-notes-ui-design.md`）。
