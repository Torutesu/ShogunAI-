...
- Then: AXツリーの有界走査(深さ8 / 300要素 / 32KB / 250msタイムボックス)でテキストのみが収集され、event_logに保存される。画像・スクリーンショットは生成されない（Visual recall を On にした場合のみ、テキストが取れなかったウィンドウで72時間保持のフレームが例外的に生成される）。ツールバー等のchromeは後回しにされ、本文が予算内で優先される
