...
6. **オーバーレイの操作性**（前回の実機ブロッカー）: カードのボタン（モード切替・Dismiss）が**押せる**こと。表示中は**透明な余白も含めてウィンドウ全体がクリックを取る**のが現在の意図した挙動なので、余白のクリックが背後の Meet に抜けないのは正常（`meeting.rs:913` のコメント参照）。非表示に戻ったら背後が操作できること
...
### 5.3 オーバーレイのマウス処理（差分 #4 → #7 で方針転換。前回の実機ブロッカー）

⚠️ **コミット履歴と最終状態が食い違う箇所。レビュー時は最終状態を見ること。**

- 差分 #4（`752612f`）は「webview の hit-test で `.ov` の上だけクリックを取る」実装だった。
- 差分 #7（`0743193`）でこれを**やめている**。理由は `meeting.rs:913-916` のコメント: pointermove ベースの hit-test は初回 move 前や pointerleave で false に落ち、クリックが背後の Meet に抜けてしまった。
- **現在の意図した挙動**: オーバーレイ表示中は `ignoresMouseEvents=false` を**ウィンドウ全体**に適用（透明な余白も含めてクリックを取る）。非表示で `true` に戻す（`meeting.rs:904-918`）。

確認:
1. カードのボタン（モード切替・Dismiss・Start）が**押せる**。
2. カードのヘッダー/グリップ/フッターをドラッグ → オーバーレイが動く（本文・アクション・モード/言語メニューの上ではドラッグしない）。
3. オーバーレイが消えたあと、背後のアプリが普通に操作できる。
4. 余白のクリックが背後に抜けないのは**現状は正常**（トレードオフを受容した設計）。オーバーレイは画面右上に park されるので、そこに Meet のコントロールが重なると押せなくなる — 実機で許容範囲か判断すること。

**残存デッドコード（レビュー対象）**: `meeting_overlay_set_interactive` コマンドと `meeting_overlay_surface` イベントは #4 の遺産で、**フロントエンドから一切呼ばれていない**。したがってログ行 `[meeting] overlay interactive=…`（`meeting.rs:991`）は**出ない**。`[meeting] overlay show … interactive=true`（表示時の1行）だけが出る。
