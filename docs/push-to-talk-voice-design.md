...
現在の `AgentClient::complete()` は非ストリーミングで、`inline_source.rs:948` にも「非ストリーミングなので初トークンではなく全体を計測している」と明記されている。SLO「アクション実行→初トークン 1s（ストリーミング必須）」を満たすには、Anthropicクライアントに SSE ストリーミングを足す必要がある。
...
### 権限
- `Info.plist` に `NSMicrophoneUsageDescription` を追加（未確認のため要確認・不足なら追加）
...
- `Info.plist` の `NSMicrophoneUsageDescription` の有無（M6着手時に確認し、無ければ追加）
