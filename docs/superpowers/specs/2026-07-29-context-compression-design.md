...
> - **配線済み（レビュー修正増分 2026-07-29 / #63）**: ①圧縮パスの facts 二重ロード解消（`assemble_evidence` 抽出。`assemble_context` は byte 一致で不変）。②`COMPRESS_BUDGET_MS` guard を支配コスト＝evidence 検索の直後へ前倒し（＋compress 前の最終 guard 保持）で総処理時間を実際に縛る。③session 要約 consume の N+1 を 1 クエリ `summaries_for_sessions` に一括化。④consume 済み ThreadSummary と同一 `thread_key` の SessionSummary を重複排除。⑤`ScoreInputs` リテラルを名前付き module const に集約（値不変）。⑥`session_ids_for_events`/`summaries_for_sessions` に決定的 `ORDER BY`。
> - **休眠（次周）**: 設定 UI トグル（現状 env フラグ）、reply ドラフト経路（`build_reply_context`）の圧縮、Batch 抽象要約器の on-device 配線、AB ダッシュボード UI、パーソナライズ、session/thread 候補のクエリ由来 relevance（現状固定）。
> - **受容済み（軽微・意図的な繰り延べ）**: ①`session::set_summary` は存在しない session id に対して silent no-op（rows-affected 未検証。best-effort 設計＝#63 finding #6、修正しない）。②Dream Cycle 側 thread-summary populate の per-thread ループ（クエリ時 consume の N+1 は解消済みだが、populate 経路は N+1 のまま。夜間バッチで許容）。
