...
- 既定ではスクリーンショットを保存しない（AXテキストのみ、不変条件2）。Visual recall を On にした場合のみ、AXテキストが空のウィンドウについて、選択した保持期間（既定3日、期限削除は常時有効）で自動削除されるフレームを保持する（2026-08-02 例外、2026-08-20 更新）
