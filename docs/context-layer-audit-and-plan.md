...
- [ ] 評価セット（質問→期待される根拠イベント）を作り recall@k を計測
      （**§9 を受けて最優先**。rank-1 ではなく top-k を主指標にする）
- [ ] recall@k が不足するならリランカーを検討（§9 の選択肢表）
...
| 案 | 効果 | コスト |
|---|---|---|
| **クロスエンコーダのリランカー**（top-20を並べ替え） | この失敗クラスの本命 | モデル追加（数百MB）＋CPU。SLO 500ms への影響を要計測 |
| クエリ拡張（読む側のモデルで問いを言い換えて複数検索） | 中程度。追加モデル不要 | レイテンシとBYOK消費 |
| 大きい埋め込みモデル（e5-base / large） | 限定的。同じ性質の問題は残る | サイズ・CPU |

**判断待ち**。まず Phase Q の recall@k 評価セットを作り、「top-k に入っているか」の実測値を
出してから決める。rank-1 が取れないこと自体は上記の契約上、必ずしも問題ではない。
