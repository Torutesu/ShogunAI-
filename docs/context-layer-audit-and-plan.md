...
### Phase R2 — 日本語抽出 + スレッド
- [ ] `extract.rs` に日本語の約束/依頼/待ち cue を追加（「〜します」「〜までに」「お願いします」「確認します」等）
- [ ] `thread_key` マイグレーション + 各コネクタでのthread_key付与
- [ ] `threads` テーブル + salience 計算

### Phase R3 — 埋め込み（意味検索）
- [ ] `ort` + `tokenizers` 依存追加、multilingual-e5-small をバンドル
- [ ] `Embedder` の実装 + 非同期embed job をdesktopで起動
- [ ] `search_hybrid` に query embedding を渡す
...
### Phase R5 — コア体験
- [ ] Reply Context Pack の事前組み立て（フォーカス変化で更新、SLO計測付き）
- [ ] 参照解決（「あの件」）+ 曖昧なら聞き返すUI
- [ ] 根拠表示（この回答の出典）
...
1. **音声はv1.5** — 参照解決（テキスト）を先に作り、ASRは後段でよいか
2. **DB暗号化** — SQLCipher導入は既存DBの移行が必要。v1に入れるか
3. **AIツール履歴の取り込み** — 現要件のスコープ表に無い新ソース。v1に足すか、v1.5送りか
4. **「ユーザーごと」の意味** — 端末内単一ユーザー（現行前提）で良いか、将来チーム共有を見据えるか
