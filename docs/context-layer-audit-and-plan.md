...
#### 実測値（40,000イベント / 400スレッド、WALファイル、CI用Linuxマシン）

| 経路 | p50 | p95 | 予算 | 判定 |
|---|---|---|---|---|
| `build_reply_context`（押下前の事前組み立て） | 0.3ms | 0.3ms | 150ms | 余裕 |
| `resolve_referent`（「あの件」の解決） | 0.1ms | 0.2ms | 150ms | 余裕 |
| `assemble_context`（質問→検索→根拠） | 173ms | **208ms** | 500ms | 予算内だが**支配的**。ログ増加で伸びる |

`assemble_context` が唯一予算の実質的な割合（約4割）を使う経路。10万件超で予算に接近する
可能性があるため、伸び方の監視対象。

- [ ] **残: 実機（Apple Silicon）での実測** — 上記は開発機の数値。SLOの最終判定は実機
