...
### 3.6 自律エージェント・ランタイム（Hermes / Nous Research 系）【2026-08-22 追加】

**彼らの世界観**: エージェントは自前のランタイム・メモリ・スケジューラを持ち、あなたのサーバーで何週間も走り続け、使うほど賢くなる。Hermes（Nous Research、2026-02公開、MIT）が代表。難しい問題を解くとスキル文書として保存し、二度と忘れない。「Not a chatbot. Not a copilot. An agent.」

**まず認めること（この相手を雑に扱わない）**: 自律実行・自己改善ループ・ローカル保存という思想は本物で、方向はShogunAIと同じ側を向いている。5つの壁のうち「実行の壁」を彼らは越えている。**最も近い"実行側"の隣人**であり、コミュニティも大きい。名指しで劣位を語ると自分が安く見える相手。

**構造的限界（それでも越えられないのはコンテキストの壁）**: Hermes の記憶は**Hermes 自身のセッションの記録**である。会話したこと、実行したタスク、解いた問題の手順（procedural memory＝「どうやるか」）。つまり**あなたがエージェントに渡した分**しか知らない。あなたの仕事の大半——読んだスレッド、出た会議、画面上で下した判断——はエージェントのセッションの外で起きる。使い込むほど「腕」は上がるが、**今日あなたに何が起きているか**は初日と同じくゼロから聞くしかない。育つのはスキルであって、状況ではない。

**ShogunAIの答え**: 記憶の系が違う。ShogunAI が持つのは situational memory＝「いま何が動いているか」（people / projects / commitments / open_loops、根拠と確度つき）で、これは**あなたが誰にも何も指示しなくても**一日から受動的に育つ。そして MCP で開いているため、**Hermes からも読める**。ここが戦略上の要点: 自律エージェントが増えるほど、彼らが装着するコンテキスト層の価値が上がる（§4.5 の堀5「アプリレイヤーは敵ではなく将来の顧客」の実行版）。

**一句（内部用）**: *Hermes remembers how. ShogunAI knows what's going on.*（Hermesは「やり方」を覚える。ShogunAIは「いま何が起きているか」を知っている）

**コピー（EN）**:
> Agent runtimes learn from their own sessions — every task you hand them makes them better at doing. But your work doesn't happen inside their sessions. ShogunAI holds the state of your actual day, and hands it to any agent over MCP. They bring the hands; we bring the world.

**コピー（JA）**:
> エージェント・ランタイムは自分のセッションから学びます。タスクを渡すほど「やり方」は上手くなる。でも、あなたの仕事はエージェントのセッションの中では起きていません。ShogunAIは実際の一日の状態を持ち、MCP経由でどのエージェントにも渡します。手足は彼らが、世界はこちらが持つ。

**併記の規律**: この比較を書くときは必ず「補完関係」で締める（敵対で締めない）。ShogunAI 自身も L1/L2/L3 の実行系を持つが、対Hermesの文脈で強調すべきは**実行の優劣ではなく記憶の系の違い**。実行で張り合うと「もう一つのエージェント」に見え、層の主張が消える。
