...
| G2 | 不変条件2: 既定で画像を保存しない | 画面キャプチャAPIの呼び出しが `visual-recall-ocr` feature 配下に限定され、Off のとき1枚も書き込まれないことを検査。保持期間72時間・自動削除・クラウド送信なしも合わせて検証（2026-08-02 例外）。音声ファイル書き出しの不存在は例外なしでgrep検査 |
