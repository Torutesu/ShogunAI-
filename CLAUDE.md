...
2. **画像・音声データを一切保存しない。** 画面キャプチャはAccessibility API経由のテキストのみ。会議の音声は原則オンデバイス処理とし、**SHOGUN 自身は波形をディスク・一時ファイルに書かない**。永続化するのは文字起こしテキストとそのprovenanceのみ。スクリーンショット・録画・音声ファイルを生成するコードを書かない  
   **【2026-08-02 明示的例外｜2026-08-20 更新】Visual recall が On のとき、OCR 用に取得した画面は圧縮 JPEG として暗号化済みメモリ DB（`screen_frames`）に、ユーザーが選択した有限期間だけ保持する。プリセット1〜7日＋上限付きカスタム（最大3,650日）、既定・フォールバックは3日。期限による自動削除は常時有効で、無効化する設定を作らない。バイト予算による早期削除もしない: 暗号化ストアが上限（2GiB）に達したら既存フレームを消さずに新規取得を停止する。保持期間中のフレームはローカルで閲覧・削除できる。クラウド送信なし・音声対象外。Visual recall Off では画像を1枚も保存しない。保持設定は desktop / MCP / REST / CLI で対称。**  
