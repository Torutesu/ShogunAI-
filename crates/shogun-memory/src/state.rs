...
// caller's concern; these functions only take the cut-off timestamps.
...
/// Open loops that appeared since `since` and are still open (Wrap "Loose ends"). Newest first is
/// not needed — staleness order matches the rest of the product.
