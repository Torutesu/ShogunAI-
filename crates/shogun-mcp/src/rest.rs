...
pub fn act(body: Option<&str>, now_ms: i64, approvals: &mut ApprovalQueue) -> (u16, String) {
    ...
    match parse_action(body) {
        ...
        Some(ActionSpec::Send(send, preview)) => {
            ...
            (
                202,
                format!(r#"{{"pending":true,"approval_id":{},"level":"L3"}}"#, id.0),
            )
        }
    }
}
...
mod tests {
    ...
    fn unknown_path_is_404_even_with_a_token() {
        assert_eq!(
            route(&req(Method::Get, "/v1/nope", Some("t")), &reg()),
            Routed::NotFound
        );
    }
    ...
    fn wrong_method_is_405() {
        // search is GET-only
        assert_eq!(
            route(&req(Method::Post, "/v1/memory/search", Some("t")), &reg()),
    ...
    fn tool_endpoints_require_a_token_including_reads() {
        assert_eq!(
            route(&req(Method::Get, "/v1/memory/search", None), &reg()),
            Routed::Unauthorized
        );
        assert_eq!(
            route(&req(Method::Get, "/v1/state/people", Some("wrong")), &reg()),
            Routed::Unauthorized
        );
    }
    ...
    fn status_is_unauthenticated() {
        assert_eq!(
            route(&req(Method::Get, "/v1/status", None), &reg()),
            Routed::Status
        );
    ...
    fn metrics_is_unauthenticated_and_get_only() {
        ...
        assert_eq!(
            route(&req(Method::Get, "/v1/metrics", None), &reg()),
            Routed::Metrics
        );
        assert_eq!(status_code(&Routed::Metrics), 200);
        // still GET-only
        assert_eq!(
            route(&req(Method::Post, "/v1/metrics", Some("t")), &reg()),
            Routed::MethodNotAllowed
        );
    }
    ...
    fn actions_execute_routes_to_action() {
        assert_eq!(
            route(&req(Method::Post, "/v1/actions/execute", Some("t")), &reg()),
            Routed::Action
        );
    ...
    fn calendar_action_requires_start_and_end_and_preserves_wire_fields() {
        ...
        let ActionSpec::Send(action, preview) = parsed else {
            panic!("expected calendar send")
        };
        assert_eq!(
            action,
        ...
        assert!(preview
            .full_body
            .contains("startTime: 2026-08-13T10:00:00Z"));
    ...
    fn actions_poll_routes_by_approval_id() {
        assert_eq!(
            route(&req(Method::Get, "/v1/actions/poll/7", Some("t")), &reg()),
            Routed::ApprovalPoll { id: 7 }
        );
    }
    ...
    fn resolved_read_tool_is_actually_a_read() {
        ...
        if let Routed::Read { tool, .. } =
            route(&req(Method::Get, "/v1/state/open_loops", Some("t")), &reg())
        {
