...
//!
//! This layer corrects only explicit, user-confirmed aliases. It deliberately does not use
//! fuzzy, phonetic, or edit-distance matching: a missed correction is safer than changing a name.
...
impl VoiceDictionary {
    ...
    pub fn correct(&self, input: &str, context: &DictionaryContext) -> DictionaryCorrection {
        ...
        while i < tokens.len() {
                ...
                .filter_map(|entry| {
                    entry
                        ...
                        .filter_map(|alias| {
                            ...
                            (alias_tokens.len() > 0
                                && i + alias_tokens.len() <= tokens.len()
                                && tokens[i..i + alias_tokens.len()]
                                    .iter()
            ...
            let end = tokens[i + alias_tokens.len() - 1].end;
...
mod tests {
    ...
    fn skips_ambiguous_aliases() {
        let mut first = entry("Alpha", &["alpha"]);
