...
mod tests {
    ...
    fn context_pack_returns_facts_and_cited_evidence() {
        ...
        let db = Db::open_in_memory(Arc::new(|| 100)).unwrap();
        let (e, _) = db.capture(&shogun_memory::event_log::NewEvent {
            ts: 1,
            source: "gmail",
            kind: "message",
            app_bundle_id: None,
        ...
        let items = backend
            .read(Tool::MemoryGetContextPack, &ReadParams { id: None, query: Some("vendor renewal".into()) });
        ...
        assert!(backend.read(Tool::MemoryGetContextPack, &ReadParams::default()).is_empty());
    }
