...
impl ExclusionPolicy {
    ...
    /// policy. Onboarding renders these directly (issue #6, step 2): a UI that hardcoded the list
    /// would tell the user a lie about today's behaviour the moment a default is added. The first
    /// four are the non-removable defaults counted from their live tables; `sensitive_titles` is
    /// the user's own title/URL patterns. Ids match the string-catalogue keys in `strings.ts`.
